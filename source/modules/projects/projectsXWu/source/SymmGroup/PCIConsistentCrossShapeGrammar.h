//---------------------------------------------------------------------------
#ifndef PCI_CONSISTENT_SHAPE_GRAMMER_H
#define PCI_CONSISTENT_SHAPE_GRAMMER_H
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
...
namespace NAMESPACE_VERSION {
    ...
    * Settings 
    **/
    class PROJECTSXWU_API ConsistentCrossShapeGrammarSettings : public AttachedData
    {
        DEFINE_CLASS(ConsistentCrossShapeGrammarSettings);
    public:
        ConsistentCrossShapeGrammarSettings();
        ~ConsistentCrossShapeGrammarSettings();

        static const char* getDefaultName() {return "ConsistentCrossShapeGrammarSettings";}

        int numCPUCores;
        std::string shapeDBPath;
    ...
    **/
    class PROJECTSXWU_API PCIConsistentCrossShapeGrammar : public PCInteractionTool
    {
        DEFINE_CLASS( PCIConsistentCrossShapeGrammar )
    private:
        unsigned	m_ScreenWidth, m_ScreenHeight;
        ConsistentCrossShapeGrammarSettings* m_Settings;
        ...
        //std::vector<CCSGSymmetryGroup*> combineFoundGroups(UnstructuredInCorePointCloud* shape, std::vector<CCSGSymmetryGroup*> groups);
        ...
        pair<UnstructuredInCorePointCloud*, CCSGClassification*> findPreliminarySymmetryGroups(const std::string& shapeName, UnstructuredInCoreTriangleMesh* selcloud, FeatureSet* fs, DVectorUObjectList* clusters);
        pair<UnstructuredInCorePointCloud*, std::vector<UnstructuredInCorePointCloud*>> extractFinalSymmetricGroups(UnstructuredInCorePointCloud* shape, FeatureSet* _fs, DVectorUObjectList* _clusters, CCSGClassification* ccsg, InCorePCTopologyGraph* tpg);
        ...
        /*struct EmbNodePair
        {
        EmbNodePair(card32 n, card32 s) : nid(n), sid(s) {}
        EmbNodePair(const EmbNodePair& emb) : nid(emb.nid), sid(emb.sid) {}
        card32 nid;
        card32 sid;
        bool operator<(const EmbNodePair& e) const
        {
        if (sid > e.sid) return false;
        else if (e.sid > sid) return false;
        //if (e.sid > sid) return false;
        return e.nid < nid;
        }
        bool operator==(const EmbNodePair& e) const { return e.nid == nid && e.sid == sid; }
        };*/
        //std::map<card64, card32> embNodeMap;
        //card32 numEmbeddedNodes;
    ...
    protected:
        PCIConsistentCrossShapeGrammar();
        ~PCIConsistentCrossShapeGrammar();
        void execute(void);

        void attachSettingsToScene();

        void loadPrincetonBenchmarkClasses();
        void loadSelectedClass();
        void importMesh();
        void segmentObjectsFromClass();
        void computeGraphs();
        //void alignTwoSelectedShapes();
        void computeSymmetryBetweenParts();
        void addEmptyClassification();

#if 0 //manual selection of segments
        void startSymmetrySelection();
        void saveAndNextBlock();
        void finishSymmetrySelection();
        void runAutomaticSymmetrySelection();
        void selectGenerator();
#endif

        void exportSymmetries();
        void computeRelationMatrices();
        void loadAndShowExportedSymmetries();

        void showMatchingForSelectedNode();
        void showRegionMatchForSelectedNode();
        ...
        void debugShowAllPreliminaryGroups();
        void applyColorsToSelectedSymmetries();
        //void debugRenderCluster();

        void showStatisticsAndLogs();

        void exportTransformations(void);
