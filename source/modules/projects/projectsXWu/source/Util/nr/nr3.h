...
struct NRerror {
  ...
  char const* file;
  int line;
  NRerror(char const* m, char const* f, int l)
...
//    if std::vector and rhs were different sizes, std::vector
...
#else
//this is many things, but not standard-conforming
...
typedef std::complex<double> nr_Complex; // default complex type
...
static const nr_Doub NaN = std::numeric_limits<nr_Doub>::quiet_NaN();
