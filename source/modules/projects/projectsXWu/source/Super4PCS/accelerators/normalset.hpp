...
{      
  ...
  return Super4PCS::Utils::UnrollIndexLoop<VALIDATE_INDICES>( coordinatesPos(p),  dim-1,  _egSize );
}
  ...
  const Point& n) const
{
  ...
  return Super4PCS::Utils::UnrollIndexLoop<VALIDATE_INDICES>( coordinatesNormal(n), dim-1, _ngSize);
}
...
{      
  // Unroll the loop over the different dimensions at compile time
  return Super4PCS::Utils::UnrollIndexLoop<VALIDATE_INDICES>( pCoord,  dim-1,  _egSize );  
}
...
{
  // Unroll the loop over the different dimension at compile time
  return Super4PCS::Utils::UnrollIndexLoop<VALIDATE_INDICES>( nCoord,  dim-1, _ngSize);
}
