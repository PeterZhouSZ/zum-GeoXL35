...
class Point3D : public Eigen::Vector3f {
 public:
  inline Point3D(double x, double y, double z) : Eigen::Vector3f(x, y, z) {}
  inline Point3D(const Eigen::Vector3f& other): Eigen::Vector3f(other) {}
  inline Point3D(const Point3D& other):
      Eigen::Vector3f(other),
      normal_(other.normal_),
      rgb_(other.rgb_),
      hsv_(other.hsv_) {}
  inline Point3D() : Eigen::Vector3f(0.0f, 0.0f, 0.0f) {}
  inline const Eigen::Vector3f& rgb() const { return rgb_; }
  inline const Eigen::Vector3f& hsv() const { return hsv_; }
  inline const Eigen::Vector3f& normal() const { return normal_; }
  inline void set_rgb(const Eigen::Vector3f& rgb) {
      rgb_ = rgb;
      internal::RGB2HSV(rgb_[0]/255.f,rgb_[1]/255.f,rgb_[2]/255.f, hsv_[0],hsv_[1],hsv_[2]);
  }
  inline void set_normal(const Eigen::Vector3f& normal) {
      normal_ = normal.normalized();
  }

 private:
  // Normal.
  Eigen::Vector3f normal_{0.0f, 0.0f, 0.0f};
  // Color.
  Eigen::Vector3f rgb_{-1.0f, -1.0f, -1.0f};
  Eigen::Vector3f hsv_{-1.0f, -1.0f, -1.0f};
};
